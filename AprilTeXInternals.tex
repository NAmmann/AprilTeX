...
\newcommand{\GenerateAprilTag}[4]{%
    ...
    %
    % Draw black border
    \fill[black] (\AptilTagOrigin, \AptilTagOrigin) rectangle (\AptilTagDimension * \AptilTagSquareSize + \AptilTagOrigin, \AptilTagDimension * \AptilTagSquareSize + \AptilTagOrigin);%
    %
    % Draw payload
    ...
    \foreach \b [count=\i from 0] in {#4} {%
        ...
        \ifthenelse{\b=1} {%
            \fill[white] (\AprilTagTempCoordinateX, \AprilTagTempCoordinateY) rectangle (\AptilTagSquareSize + \AprilTagTempCoordinateX, \AptilTagSquareSize + \AprilTagTempCoordinateY);%
        }{}%
    }%
    %
    % Put label
